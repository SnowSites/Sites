...
async function loadUsersPublicOnce() {
  const now = Date.now();
  if (__allTagsCache && (now - __allTagsLoadedAt) < 5 * 60 * 1000) return __allTagsCache; // 5 min cache
  try {
    const snap = await get(ref(db, 'tags'));
    const obj = snap.exists() ? snap.val() : {};
    __allTagsCache = Object.keys(obj || {});
    __allTagsLoadedAt = now;
    return __allTagsCache;
  } catch (e) {
    console.warn('loadUsersPublicOnce failed', e);
    __allTagsCache = [];
    __allTagsLoadedAt = now;
...
const toggleView = (viewName) => {
    ...
    if (viewName === 'history') {
      ...
      try { renderOnlineUsers(); setInterval(renderOnlineUsers, 5000); } catch(e) {}
        adminLoadRanking(); });
