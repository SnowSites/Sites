            ...
            .then(userCredential => {
                ...
                return updateProfile(user, { displayName: displayName })
                    .then(() => {
                        ...
                        return set(userRef, newUserProfile).then(() => ensureTagIndex(user.uid, displayName)).then(() => ensureTagIndex(user.uid, displayName));
                    });
            })
            .catch((error) => {
                const code = error.code;
...
async function adminCreateFaccao() {
  ...
  if (els.adminNewFaccaoLeaderTag) els.adminNewFaccaoLeaderTag.value = '';
...
async function leaderAddMember() {
  const fid = getFaccaoId();
  const tag = (els.leaderAddMemberTag && els.leaderAddMemberTag.value) ? els.leaderAddMemberTag.value.trim() : '';
  const uid = tag ? await uidFromTag(tag) : '';
  ...
  await set(ref(db, `usuarios/${uid}/faccaoId`), fid);
  await set(ref(db, `faccaoMembros/${fid}/${uid}/cargo`), cargo);
  if (els.leaderAddMemberTag) els.leaderAddMemberTag.value='';
